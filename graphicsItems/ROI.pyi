...
class ROI(GraphicsObject):
    ...
    **Signals**
    ...
    def saveState(self):
        ...
        restored."""
    def setState(self, state: dict[str, float | Point | QtCore.QPointF | tuple[float, float] | list[float]], update: bool = True) -> None:
        """
    ...
    def addScaleHandle(self, pos: tuple[float, float]|list[float], center: tuple[float, float]|list[float], axes: int  | tuple[int, int] | tuple[int, int, int] | None = None, item: Handle | None = None, name: str | None = None, lockAspect: bool = False, index: SupportsIndex | None = None) -> Handle:
        ...
        **Arguments**
        ...
        =================== ====================================================
        """
    def addRotateHandle(self, pos: tuple[float, float]|list[float], center: tuple[float, float]|list[float], item: Handle | None = None, name: str | None = None, index: int | None = None) -> Handle:
        """
    ...
    def getArraySlice(self, data: NDArray[np.uint8, np.int64, np.float64], img: ImageItem, axes=(0, 1), returnSlice: bool = True):
        ...
        """
    def getArrayRegion(self, data: NDArray[np.uint8, np.int64, np.float64], img: ImageItem, axes=(0, 1), returnMappedCoords: bool = False, **kwds : NDArray[np.uint8, np.int64, np.float64] | tuple[int |float, ...] | list[int |float] | tuple[tuple[int | float, ...], ...] ) -> NDArray[np.uint8, np.int64, np.float64] | tuple[NDArray[np.uint8, np.int64, np.float64], NDArray[np.uint8, np.int64, np.float64]]: """Use the position and orientation of this ROI relative to an imageItem
